...
class InitiatePayment(InterfaceBase):
    ERROR_CODES = {
    ...
    def _get_balance_entry_type(cls) -> Optional[Any]:
        if cls._balance_entry_type_cache is None:
            cls._balance_entry_type_cache = BalanceEntryTypeService().get(name="InitiatePayment")
        ...
        try:
            with transaction.atomic():
                ...
                try:
                    ...
                    log.exception("Failed to initiate transaction: %s", e)
                    return self.ERROR_CODES['TRANSACTION_CREATION_FAILED']
                try:
                    ...
                    return self.ERROR_CODES['TRANSACTION_EXECUTION_FAILED']
                log.info(
                    "Successfully initiated payment for contribution %s, amount %s, reference %s",
                    contribution.id,
                    amount,
                    ref
                )
            ...
            log.exception("Unexpected error in transaction processing: %s", e)
            if transaction_history:
                try:
    ...
    def post(self, contribution_id: int, request=None, **kwargs) -> Dict[str, Any]:
        try:
            ...
            return self._process_transaction(contribution, amount, phone_number, kwargs['ref'], kwargs['charge'])
        except Exception as e:
            log.exception(
...
class ApprovePaymentTransaction(InterfaceBase):
    _balance_entry_type_cache: Optional[Any] = None
    _active_state_cache: Optional[Any] = None
    ...
    def _get_balance_entry_type(cls) -> Any:
        if cls._balance_entry_type_cache is None:
            cls._balance_entry_type_cache = BalanceEntryTypeService().get(name="ApprovePaymentTransaction")
        return cls._balance_entry_type_cache
    ...
    def post(self, request, reference: str, receipt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            log.exception("ApprovePaymentTransaction: post exception for reference %s: %s", reference, ex)
            return {"code": "500.001", "message": "Transaction approval failed due to system error"}


class RejectPaymentTransaction(InterfaceBase):
    _balance_entry_type_cache: Optional[Any] = None
    _active_state_cache: Optional[Any] = None
    ...
    def _get_balance_entry_type(cls) -> Any:
        if cls._balance_entry_type_cache is None:
            cls._balance_entry_type_cache = BalanceEntryTypeService().get(name="RejectPaymentTransaction")
        return cls._balance_entry_type_cache
    ...
    def post(self, request, reference: str, status: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                with trx.atomic():
                    description = (
                        f"Contribution rejected for {account.contribution.name} "
                        f"with reference {transaction_history.reference}"
                    )
                    ...
                    if not rejected_transaction:
                        raise Exception("Failed to reject transaction")
            ...
            log.exception("rejecteTopupTransaction: post exception for reference %s: %s", reference, ex)
            return {"code": "500.001", "message": "Transaction approval failed due to system error"}
