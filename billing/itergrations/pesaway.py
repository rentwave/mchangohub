...
class PesaWayAPIClient:
	...
	async def _make_request(
		...
		if current_requests > settings.PESAWAY_RATE_LIMIT_PER_MINUTE:
			logger.warning("Rate limit exceeded", client_id=self.client_id)
			return APIResponse(
				success=False,
		...
		async with self.semaphore:
			...
			try:
				...
				if response.status == 401 and retries < 1:
					logger.info("Token expired, refreshing", request_id=request_id)
				...
				logger.error("Circuit breaker open", endpoint=endpoint, error=str(e))
				return APIResponse(
					success=False,
	...
	async def send_mobile_money(
		...
		return await self._make_request('POST', '/api/v1/mobile-money/send-payment/', payload)
	...
	async def receive_c2b_payment(
		...
		payload = {
			...
			"Amount": amount,
			"PhoneNumber": phone_number,
			"Channel": "MPESA",  # C2B payments are typically MPESA
			"Reason": reason,
			"ResultsUrl": results_url
		}
		return await self._make_request('POST', '/api/v1/mobile-money/receive-payment/', payload)
	
	async def authorize_transaction(self, transaction_id: str, otp: str) -> APIResponse:
		"""Authorize transaction with OTP"""
		payload = {
			"TransactionID": transaction_id,
			"OTP": otp
		}
		return await self._make_request('POST', '/api/v1/mobile-money/authorize-transaction/', payload)
	...
	async def send_bank_payment(
		...
		return await self._make_request('POST', '/api/v1/bank/send-payment/', payload)
	
	async def query_bank_transaction(self, transaction_reference: str) -> APIResponse:
		"""Query bank transaction status"""
		payload = {"TransactionReference": transaction_reference}
		return await self._make_request('POST', '/api/v1/bank/transaction-query/', payload)
	
	async def query_mobile_money_transaction(self, transaction_reference: str) -> APIResponse:
		"""Query mobile money transaction status"""
		payload = {"TransactionReference": transaction_reference}
		return await self._make_request('POST', '/api/v1/mobile-money/transaction-query/', payload)
	...
	async def pull_mobile_money_transactions(
		...
		"""Pull mobile money transactions for a date range"""
		payload = {
		...
		return await self._make_request('POST', '/api/v1/mobile-money/pull-transactions/', payload)
	...
	async def send_airtime(
		...
		return await self._make_request('POST', '/api/v1/airtime/send-airtime/', payload)
	
	async def batch_send_mobile_money(self, payments: List[Dict]) -> List[APIResponse]:
		"""Send multiple mobile money payments concurrently"""
		tasks = []
