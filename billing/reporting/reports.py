...
def download_content_type(extension=None):
    ...
    try:
        ...
        if extension:
            return lookups.get(extension, "application/octet-stream")
        return lookups
    except Exception as e:
        logger.exception("download_content_type Exception: %s", e)
    return None
...
class StatementGenerator:
    ...
    @csrf_exempt
    def generate_statement(self, request) -> HttpResponse:
        """
        ...
        with open(file_path, "rb") as f:
            response = HttpResponse(
                f.read(),
                content_type=download_content_type(file_path.split('.')[-1])
            )
