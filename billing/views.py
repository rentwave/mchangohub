...
def calculate_fair_tiered_charge(amount_kes):
    """
    Calculates a fair, tiered transaction charge for any amount up to and beyond 10 million KES.
    Charges are progressive based on defined tiers.
    """

    tiers = [
        (0, 1000, 0.005),          # 0.5%
        (1001, 10000, 0.01),       # 1%
        (10001, 100000, 0.015),    # 1.5%
        (100001, 500000, 0.02),    # 2%
        (500001, 1000000, 0.025),  # 2.5%
        (1000001, 5000000, 0.03),  # 3%
        (5000001, 10000000, 0.04), # 4%
    ]
    if amount_kes > 10000000:
        return round(amount_kes * 0.05, 2)
    charge = 0.0
    for lower, upper, rate in tiers:
        if amount_kes > lower:
            applicable_amount = min(amount_kes, upper) - lower
            charge += applicable_amount * rate
...
class PesaWayWalletInterface:
    ...
    def b2c_transfer(self, request):
        ...
        try:
            ...
            charge = calculate_fair_tiered_charge(round(float(data.get("amount")), 2))
            amount_plus_charge = round(float(data.get("amount")), 2) + charge
